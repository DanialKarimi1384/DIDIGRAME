...
export async function register(request, env) {
  ...
  await env.DB.prepare(
    "INSERT INTO users (id, username, password_hash, salt, bio, created_at) VALUES (?, ?, ?, ?, '', ?)"
  )
    .bind(userId, username, passwordHash, salt, now)
    .run();

  // ساخت خودکار چت «پیام‌های ذخیره‌شده» برای کاربر جدید
  const savedChatId = uid();
  await env.DB.batch([
    env.DB.prepare(
      "INSERT INTO chats (id, type, name, created_by, created_at) VALUES (?, 'saved', 'پیام‌های ذخیره‌شده', ?, ?)"
    ).bind(savedChatId, userId, now),
    env.DB.prepare(
